...
outdir=$(pwd)
codeDir="/exp/emph/app/users/$USER/emph_12.4.23"
buildDir=${CETPKG_BUILD}
tarFileName="emphaticsoft_${timestamp}.tar.gz"
safeScratchDir="/exp/emph/app/users/$USER/emph_12.4.23"
wrapperFileName="runEMPHATICSimOnGrid_${timestamp}.sh"
gridNodeOutDir="OUTPUT"
...
if [[ ! ${outdir} =~ "/pnfs/emphatic/scratch/users/${USER}" ]]
then
  echo "Output directory, ${outdir}, is not on /pnfs.  It must be visible to grid nodes.  Try running again from /pnfs/emphatic/scratch/users/${USER} this time." >&2
  exit 2
fi
...
if [[ ! -n ${CETPKG_SOURCE} ]]
then
  echo "I can't find the code directory to tar up because CETPKG_SOURCE is not set!  Set up emphaticsoft and try again." >&2
  exit 2
fi
...
if tar -czf ${safeScratchDir}/${tarFileName} -C ${codeDir} emphaticsoft build && ifdh cp -D ${safeScratchDir}/${tarFileName} ${outdir}/
then
  rm ${safeScratchDir}/${tarFileName}
else
  echo "Tarring up ${codeDir} and ${buildDir} into ${safeScratchDir}/${tarFileName} and/or copy to /pnfs failed!" >&2
  exit 3
fi
...
echo "source \${INPUT_TAR_DIR_LOCAL}/emphaticsoft/setup/setup_emphatic.sh" >> ${tmpWrapperFile}
echo "cd \${INPUT_TAR_DIR_LOCAL}/build" >> ${tmpWrapperFile}
echo "source \${INPUT_TAR_DIR_LOCAL}/emphaticsoft/ups/setup_for_development -p"  >> ${tmpWrapperFile}
...
echo "echo \${CETPKG_SOURCE}" >> ${tmpWrapperFile} #Debugging
...
echo "/usr/bin/time -v art -c ${fhiclFile} -o ${fhiclFile%.*}_\${PROCESS}.sim.root -n 5" >> ${tmpWrapperFile} #TODO: Use art -o and/or art -i to control input file on the grid?
...
jobsub_submit --group fermilab --OS sl7 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --role=Analysis --expected-lifetime ${lifetime} --memory ${memory} --tar_file_name dropbox://${outdir}/${tarFileName} --use-cvmfs-dropbox -f ${outdir}/${fhiclFile} -d ${gridNodeOutDir} ${outdir} -N 1 file://${outdir}/${wrapperFileName}
#jobsub_submit --group emphatic --OS sl7 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --role=Analysis --expected-lifetime ${lifetime} --memory ${memory} --tar_file_name dropbox://${outdir}/${tarFileName} --use-cvmfs-dropbox -f ${outdir}/${fhiclFile} -f ${outdir}/playlists.txt -d ${gridNodeOutDir} ${outdir} -N ${#playlistDirs[@]} file://${outdir}/${wrapperFileName}
